...
class LMD(LMDConstants):
    ...
    def __decrypt(self, fname, debug=False):
        ...
        self.date = linvtimelib.convert_date(struct.unpack('<H', tmp)[0])
        

        tmp = self.__LMD_data[self.LMD_TIME_OFFSET:self.LMD_TIME_OFFSET + self.LMD_TIME_LENGTH]
        self.time = linvtimelib.convert_time(struct.unpack('<H', tmp)[0])
    ...
    def __get_rc4_key(self):
        ...
        return linvrsa.crypt(e_key, self.__rsa_pu)
    ...
    def __get_md5(self):
        ...
        return linvrsa.crypt(e_md5, self.__rsa_pu)
