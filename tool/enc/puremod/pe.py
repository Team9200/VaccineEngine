        ...
        try:
            ...
            if rsrc_rva:  # 리소스가 존재한가?
                try:
                    ...
                    # Type 체크
                    num_type_name = linvsharelib.get_uint16(mm, rsrc_off+0xC)
                    num_type_id = linvsharelib.get_uint16(mm, rsrc_off + 0xE)

                    for i in range(num_type_name + num_type_id):
                        type_id = linvsharelib.get_uint32(mm, rsrc_off + 0x10 + (i*8))
                        name_id_off = linvsharelib.get_uint32(mm, rsrc_off + 0x14 + (i * 8))
                        ...
                        if type_id & 0x80000000 == 0x80000000 or type_id == 0xA or type_id == 0:
                            if type_id & 0x80000000 == 0x80000000:
                                ...
                                len_name = linvsharelib.get_uint16(mm, string_off)
                            ...
                            num_name_id_name = linvsharelib.get_uint16(mm, name_id_off + 0xC)
                            num_name_id_id = linvsharelib.get_uint16(mm, name_id_off + 0xE)

                            for j in range(num_name_id_name + num_name_id_id):
                                name_id_id = linvsharelib.get_uint32(mm, name_id_off + 0x10 + (j * 8))
                                language_off = linvsharelib.get_uint32(mm, name_id_off + 0x14 + (j * 8))
                                ...
                                num_language_name = linvsharelib.get_uint16(mm, language_off + 0xC)
                                num_language_id = linvsharelib.get_uint16(mm, language_off + 0xE)

                                for k in range(num_language_name + num_language_id):
                                    # language_id = linvsharelib.get_uint32(mm, language_off + 0x10 + (k * 8))
                                    data_entry_off = linvsharelib.get_uint32(mm, language_off + 0x14 + (k * 8))

                                    data_entry_off = (data_entry_off & 0x7FFFFFFF) + rsrc_off

                                    data_rva = linvsharelib.get_uint32(mm, data_entry_off)
                                    data_off, _ = self.rva_to_off(data_rva)
                                    if data_off > self.filesize:
                                        continue

                                    data_size = linvsharelib.get_uint32(mm, data_entry_off + 4)
            ...
            if imp_rva:  # Import API 존재
                ...
                if len(imp_data) == imp_size:
                    for i in range(imp_size / 0x14):  # DLL 정보 크기가 0x14
                        try:
                            dll_rva = linvsharelib.get_uint32(imp_data, (i*0x14)+0xC)
                            api_rva = linvsharelib.get_uint32(imp_data, (i * 0x14))
                            bo = 2
                            if api_rva == 0:
                                api_rva = linvsharelib.get_uint32(imp_data, (i*0x14)+0x10)
                                bo = 0
                            ...
                            while True:
                                try:
                                    api_name_rva = linvsharelib.get_uint32(mm, t_off)
                                except struct.error:
                                    break
            ...
            if debug_rva:  # Debug 정보 존재
                ...
                debug_off = linvsharelib.get_uint32(mm, t + 0x18)
                debug_size = linvsharelib.get_uint32(mm, t + 0x10)
            ...
            if self.verbose:
                print '-' * 79
                linvsharelib.vprint('Engine')
                linvsharelib.vprint(None, 'Engine', 'pe.kmd')
                linvsharelib.vprint(None, 'File name', os.path.split(self.filename)[-1])
                linvsharelib.vprint(None, 'MD5', linvsharelib.md5(mm[:]))

                print
                linvsharelib.vprint('PE')
                linvsharelib.vprint(None, 'EntryPoint', '%08X' % pe_format['EntryPoint'])
                linvsharelib.vprint(None, 'EntryPoint (Section)', '%d' % pe_format['EntryPoint_in_Section'])

                # 섹션 보기
                if section_num:
                    print
                    linvsharelib.vprint('Section Header')
                ...
                if section_num:
                    print
                    linvsharelib.vprint('Section MD5')
                    ...
                    for s in self.sections:
                        ...
                        size = s['SizeRawData']
                        fmd5 = linvsharelib.md5(mm[off:off+size]) if size else '-'
                        print '    %-8s %8d %s' % (s['Name'], size, fmd5)

                print
                linvsharelib.vprint('Entry Point (Raw)')
                print
                linvsharelib.HexDump().Buffer(mm[:], pe_format['EntryPointRaw'], 0x80)
                print
                if 'PDB_Name' in pe_format:
                    linvsharelib.vprint('PDB Information')
                    linvsharelib.vprint(None, 'Name', '%s' % repr(pe_format['PDB_Name']))
...
def pe_parse(mm):
    ...
    try:
        ...
        pe_pos = linvsharelib.get_uint32(mm, 0x3C)
        ...
        if mm[pe_pos + 0x18:pe_pos + 0x18 + 2] != bit32:
            if mm[pe_pos + 0x18:pe_pos + 0x18 + 2] != bit64:
                raise ValueError

        pe_ep = linvsharelib.get_uint32(mm, pe_pos + 0x28)
        pe_format['EntryPoint'] =pe_ep

        pe_img = linvsharelib.get_uint32(mm, pe_pos + 0x34)
        pe_format['ImageBase'] = pe_img

        pe_file_align = linvsharelib.get_uint32(mm, pe_pos + 0x3C)
        pe_format['FileAlignment'] = pe_file_align

        section_num = linvsharelib.get_uint16(mm, pe_pos + 0x6)
        pe_format['SectionNumber'] = section_num

        opthdr_size = linvsharelib.get_uint16(mm, pe_pos + 0x14)
        ...
        for i in range(section_num):
            ...
            section['VirtualSize'] = linvsharelib.get_uint32(mm, s+8)
            section['RVA'] = linvsharelib.get_uint32(mm, s+12)
            section['SizeRawData'] = linvsharelib.get_uint32(mm, s+16)
            section['PointerRawData'] = linvsharelib.get_uint32(mm, s+20)
            section['Characteristics'] = linvsharelib.get_uint32(mm, s+36)
